...
class TestEvent(unittest.TestCase):
    ...
    def test_When_passing_valid_kwargs_They_are_set(self):
        e = Event(service='srv', device='dev', key='key', data='data')
        self.assertEqual(e.service, 'srv')
    ...
    def test_When_matching_list_where_one_event_matches_It_matches(self):
        e1 = Event(service='srv', key='key', data='data')
        e2 = Event(service='srv', key='key', data='data2')
    ...
    def test_When_data_matches_wildcard_It_is_a_match(self):
        e1 = Event(service='srv', key='key', data={'value': 2, 'type': 'humidity'}) # occurring
        e2 = Event(service='srv', key='key', data={'value': 2, 'type': '*'})        # rule
        self.assertTrue(e1.match(e2))
    ...
    def test_When_configured_data_is_subset_of_occuring_data_with_wildcard_It_is_a_match(self):
        e1 = Event(service='srv', key='key', data={'value': 2, 'type': 'humidity'}) # occurring
        e2 = Event(service='srv', key='key', data={'value': '*'})                   # rule
        self.assertTrue(e1.match(e2))
    ...
    def test_When_deep_configured_data_is_subset_of_occuring_data_It_is_a_match_2(self):
        e1 = Event(service='srv', key='key', data={'value': 2, 'extra': {'type': 'humidity'}}) # occurring
        e2 = Event(service='srv', key='key', data={'value': 2, 'extra': {}})                   # rule
        self.assertTrue(e1.match(e2))
