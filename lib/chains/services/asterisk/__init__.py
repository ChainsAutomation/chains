...
class AsteriskReader(Thread):
    ...
    def __init__(self, service):
        ...
        # config
        try:
            mnguser = self.service.config['main']['mnguser'] 
        except KeyError:
            mnguser = 'haservice'
        try:
            mngpass = self.service.config['main']['mngpass'] 
        except KeyError:
            mngpass = ''
        try:
            mngip = self.service.config['main']['mngip'] 
        except KeyError:
            mngip = '127.0.0.1'
        try:
            mngport = self.service.config['main']['mngport'] 
        except KeyError:
            mngport = '5038'
        #  asterisk
