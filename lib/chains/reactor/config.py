...
def getData(module=None):
    ...
    ret = []
    for modname in mods:
        if module and module != modname: continue
...
def _getEnabledFiles():
    ...
    for f in _getRuleFiles():
        if f[1]: ret.append(f[0])
    return ret

def _getAvailableFiles():
    ret = []
    for f in _getRuleFiles():
        if not f[1]: ret.append(f[0])
    return ret
