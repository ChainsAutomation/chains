...
class EspeakDevice(Device):
    ...
    def onCommand(self, cmd, args):
        ...
        if cmd == 'speak':
            ...
            txt = txts[k]
        try:
            espeak(txt, config.getDeviceDataPath(self.config['id']), self)
        except Exception, e:
...
def espeak(txt, tmp, obj=None):
    ...
    if not os.path.exists(wav): # or os.lstat(wav)[6] < 400: # ignore small files (bug)
        ...
        f.close()
        pa = None
        if obj: pa = config.getDeviceSharePath(obj)
        else: pa = '/srv/chains/share/devices/Espeak'
