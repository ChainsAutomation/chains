...
class IcalHaDevice(Device):
    ...
    def onDescribe(self):
        return {
            'info': 'iCalendar device',
            'commands': [
                ('timer', [('minutes', 'int'),('label','str'),('message','str')], 'Creates a timer event from device'),
...
class IcalHaReader():
    ...
    def getconfig(self):
        print "getconfig()"
        try:
            checkinterval = self.device.config['main']['checkinterval'] 
        except KeyError:
            print 'checkinterval not in config, using defaults'

        for key in self.device.config:
            ...
            if key != 'main' and key != 'id' and key != 'extra' and key != 'logoptions' and key != 'logfiles':
                print key
                try:
                    print self.device.config[key]
                    file = self.device.config[key]['file']
                    ...
                    sys.exit(1)
                try:
                    defalarms = self.device.config[key]['defaultalarms']
                ...
                try:
                    user = self.device.config[key]['username']
                    caldict.update({'username': user})
                except KeyError:
                    self.dprint("no username defined")
                try:
                    passwd = self.device.config[key]['password']
    ...
    def reader(self):
        ...
        while (item < (len(self.alarms))):
            ...
            if(self.alarms[item][0] < now):
                ...
                self.device.onEvent(rcal, self.caldict[rcal][rtype][ruid]['SUMMARY'], self.caldict[rcal][rtype][ruid])
        ...
        while (item < (len(self.tempalarms))):
            if(self.tempalarms[item][0] < now):
                ...
                print "alarm in the past, send event"
                self.device.onEvent(self.tempalarms[item][1], 'timer', self.tempalarms[item][2])
