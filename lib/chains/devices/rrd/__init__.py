...
class RrdDevice(chains.device.Device):
    ...
    def onInit(self):
        ...
        log.info('Device started with %s plots, %s graphs, interval %s' % (len(self.plots), len(self.graphs), self.interval))
    ...
    def getPlotValue(self, plot):
        ...
        device    = source.pop(0)
        key       = source.pop(0)
        stateKey  = '%s.%s.data.%s' % (device, key, '.'.join(source))
